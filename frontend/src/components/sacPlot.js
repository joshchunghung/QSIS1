...
export function sacPlots () {
    ...
    chart.data = (vaule) => {
        ...
        const promises = paths.map((path) => {
            ...
            return axios.post('http://140.109.82.44:8012/api/test/', stationURLInfo
            ).then(success => {
                const fileName = 'test'
                const xyArr = []
                                ...
                                .on('mousemove', function (event) { // update tooltip content, line, circles and text when mouse moves
                                    ...
                                    tooltip.html(divHtml)
                                        ...
                                        .style('left', `${pointer[0] < chart_center[0]
                                            ...
                                            : event.pageX - tooltipMouseGap - tooltip.property('clientWidth')}px`)
                                        .style('top', `${pointer[1] < chart_center[1]
                                            ? event.pageY
                                            : event.pageY - tooltip.property('clientHeight')}px`)
                                        .append('div')
